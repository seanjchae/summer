...
for i in range(len(eta1)):
	...
	dRresult.append(dR)
	deltaeta=abs(eta1[i]-eta2[i])
	deta.append(deltaeta)
wp=[]
coshdeta=np.cosh(deta)
for i in range(len(wpt)):
	wp.append(wpt[i]*coshdeta[i])
...
counts,xbins,ybins,image=plt.hist2d(wp,dRresult,range=[[0,1000],[0,10]],density=True,bins=20,norm=mpl.colors.LogNorm(),cmap=mpl.cm.jet)
plt.colorbar()
